# HTML5 Semantic Elements Checklist — Assignment 1
...
| `<legend>` | `booking.html` | Line 34 | Zhaksylyk Adilet | `<legend>Guest Contact Information</leg` |
| `<label for='...'>` | `booking.html` | Line 37 | Zhaksylyk Adilet | `<label for="book-name">Full Name (requ` |
| `<input type='text'>` | `booking.html` | Line 38 | Zhaksylyk Adilet | `<input type="text" id="book-name" name` |
| `<input type='email'>` | `booking.html` | Line 42 | Zhaksylyk Adilet | `<input type="email" id="book-email" na` |
| `<input type='tel'>` | `booking.html` | Line 46 | Zhaksylyk Adilet | `<input type="tel" id="book-phone" name` |
| `<input type='date'>` | `booking.html` | Line 53 | Zhaksylyk Adilet | `<input type="date" id="book-date" name` |
| `<input type='number'>` | `booking.html` | Line 57 | Zhaksylyk Adilet | `<input type="number" id="book-guests" ` |
| `Radio group (type='radio')` | `booking.html` | Line 61 | Zhaksylyk Adilet | `<label><input type="radio" name="zone_` |
| `<select> and <option>` | `booking.html` | Line 67 | Zhaksylyk Adilet | `<select id="book-purpose" name="book_p` |
| `<textarea>` | `booking.html` | Line 76 | Zhaksylyk Adilet | `<textarea id="book-notes" name="book_n` |
| `<input type='checkbox'>` | `booking.html` | Line 80 | Zhaksylyk Adilet | `<input type="checkbox" id="book-agreem` |
| `<button type='submit'>` | `booking.html` | Line 85 | Zhaksylyk Adilet | `<button type="submit">Confirm Reservat` |
| `<button type='reset'>` | `booking.html` | Line 86 | Zhaksylyk Adilet | `<button type="reset">Clear Form</butto` |
| `Second form on feedback.html` | `feedback.html` | Line 31 | Zhaksylyk Adilet | `<form method="post" action="#">` |
| `<code>` | `colophon.html` | Line 30 | Zhaksylyk Adilet | `<!-- Why: I used <pre> and <code> to s` |
| `<pre>` | `colophon.html` | Line 30 | Zhaksylyk Adilet | `<!-- Why: I used <pre> and <code> to s` |
| `<kbd>` | `colophon.html` | Line 43 | Zhaksylyk Adilet | `<!-- Why: I used <kbd> for keyboard sh` |
| `<samp>` | `colophon.html` | Line 43 | Zhaksylyk Adilet | `<!-- Why: I used <kbd> for keyboard sh` |
| `<span> (with comment)` | `colophon.html` | Line 50 | Zhaksylyk Adilet | `<p>Current validation status: <span>Va` |
| `<hr>` | `colophon.html` | Line 51 | Zhaksylyk Adilet | `<hr>` |
| `<br>` | `colophon.html` | Line 52 | Zhaksylyk Adilet | `<p>Every element was chosen based on i` |
